...
// Terminating a process of your choice using the PID, useful if the cheat is also using a driver to strip it's handles and therefore you can forcefully close it using the driver
...
VOID MyLoadImageNotifyRoutine(IN PUNICODE_STRING FullImageName, IN HANDLE ProcessID, IN PIMAGE_INFO iMAGEiNFO)
{
...
NTSTATUS DriverEntry(_In_  struct _DRIVER_OBJECT *DriverObject, _In_  PUNICODE_STRING RegistryPath)
{
	...
	RtlInitUnicodeString(&drv_name, L"\\Driver\\SACDriver1");
